...
def save_network(light_net_e, light_net, optimizer, config, epoch):
        ...
        copy_weight = 1/layers_number*light_net.conv_curve[0].weight.data
        copy_bias = 1/layers_number*light_net.conv_curve[0].bias.data
        for i in range(layers_number-1):
            copy_weight += 1/layers_number*light_net.conv_curve[i+1].weight.data
            copy_bias += 1/layers_number*light_net.conv_curve[i+1].bias.data
        light_net_e.conv_curve.weight.data = decay*copy_weight + (1-decay)*light_net_e.conv_curve.weight.data
        light_net_e.conv_curve.bias.data = decay*copy_bias + (1-decay)*light_net_e.conv_curve.bias.data
        
        copy_weight = 1/layers_number_1*light_net.conv_curve_1[0].weight.data
        copy_bias = 1/layers_number_1*light_net.conv_curve_1[0].bias.data
        for i in range(layers_number_1-1):
            copy_weight += 1/layers_number_1*light_net.conv_curve_1[i+1].weight.data
            copy_bias += 1/layers_number_1*light_net.conv_curve_1[i+1].bias.data
        light_net_e.conv_curve_1.weight.data = decay*copy_weight + (1-decay)*light_net_e.conv_curve_1.weight.data
        light_net_e.conv_curve_1.bias.data = decay*copy_bias + (1-decay)*light_net_e.conv_curve_1.bias.data
...
def train(config):
    ...
    for epoch in range(config.num_epochs):
        ...
        for iteration, dataset in enumerate(train_loader):
            ...
            loss_smo = 100000 * L_smo(rr1) + 100000 * L_smo(rr2)  # 1000
            ...
            loss_gcol = 10 * torch.mean(L_gcolor(enhanced_image))  # 20
