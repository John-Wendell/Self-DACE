...
class light_net(nn.Module):
    ...
    def __init__(self, mode='test', layers=7):
        ...
        #########################################
            ...
            # self.conv_a = nn.ModuleList([nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True) for i in range(self.layers_low)])
            # self.conv_b = nn.ModuleList([nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True) for i in range(self.layers_low)])
            self.conv_curve = nn.ModuleList([nn.Conv2d(input_channel, 2*input_channel, 3, 1, 1, bias=True) for i in range(self.layers_low)])
            self.conv_curve_1 = nn.ModuleList([nn.Conv2d(input_channel, 2*input_channel, 3, 1, 1, bias=True) for i in range(self.layers_low)])

            # self.backbone_1 = nn.ModuleList([nn.Conv2d(number_f, number_f, 3, 1, 1, bias=True) for i in range(self.layers_high)])
            # self.conv_a_1 = nn.ModuleList([nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True) for i in range(self.layers_high)])
            # self.conv_b_1 = nn.ModuleList([nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True) for i in range(self.layers_high)])
        ###########################################
            ...
            # self.conv_a = nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True)
            # self.conv_b = nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True)
            self.conv_curve = nn.Conv2d(input_channel, 2*input_channel, 3, 1, 1, bias=True)
            self.conv_curve_1 = nn.Conv2d(input_channel, 2*input_channel, 3, 1, 1, bias=True)
            # self.backbone_1 = nn.Conv2d(number_f, number_f, 3, 1, 1, bias=True)
            # self.conv_a_1 = nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True)
            # self.conv_b_1 = nn.Conv2d(input_channel, input_channel, 3, 1, 1, bias=True)
    ...
    def forward(self, xo, mode='test'):
        if mode == 'train':
            ...
            for i in layer_order_random:
                ...
                curve_para = self.conv_curve[i](x_i)
                amp_i = self.sigmoid(curve_para[:,0:3,:,:]) * 1
                bet_i = self.sigmoid(curve_para[:,3:6,:,:])*0.7 + 0.3
            ...
            for i in layer_order_random:
                curve_para_1 = self.conv_curve_1[i](x_i)
                amp_i = -self.sigmoid(curve_para_1[:,0:3,:,:]) * 1
                bet_i = self.sigmoid(curve_para_1[:,3:6,:,:])*0.2 + 0.1
            ...
            for i in range(self.layers_low):
                ...
                curve_para = self.conv_curve(x_i)
                # curve_para_1 = self.conv_curve_1(x_i)
                amp_i = self.sigmoid(curve_para[:,0:3,:,:]) * 1
                bet_i = self.sigmoid(curve_para[:,3:6,:,:])*0.7 + 0.3
                x_i = x_i + amp_i * 1 / (1 + torch.exp(-15 * (-x_i + bet_i - 0.1))) * x_i * (bet_i - x_i) * (1 / bet_i)
